 ...
 * 作 者： gaolei
 * 日 期：2017年01月04日
 * 描 述：收到红包记录的Presenter
 */
public class ReceiveRedBagPresenter implements DataCallBack {

    private ReceiveRedBagDao dao;

    private BaseViewLayerInterface view;

    public ReceiveRedBagPresenter(BaseViewLayerInterface view) {
        this.view = view;
        dao = new ReceiveRedBagImpl(this);
    }


    public void getReceiveRedBagData( int pageStart, int type){

        Map<String, Object> param = new HashMap<>();
        param.put("pageStart", pageStart);
        param.put("pageSize", AppConstant.MAX_PAGE_NUM);

        dao.setReceiveRedBag(param, CardConstant.RECEIVE_REDBAG,type);
    }
