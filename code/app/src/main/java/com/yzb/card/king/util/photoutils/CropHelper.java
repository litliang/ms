...
public class CropHelper {
    ...
    public static Uri generateUri() {
        File cacheFolder = new File(Environment.getExternalStorageDirectory() + "/" + AppConstant.APP_DIR + "/" + AppConstant.APP_TEMP);
        if (!cacheFolder.exists()) {
            try {
                boolean result = cacheFolder.mkdir();
            } catch (Exception e) {
                LogUtil.e(TAG, "generateUri failed: " + cacheFolder);
            }
        }
        return Uri.fromFile(cacheFolder).buildUpon().appendPath("temp.jpg").build();
    }
    ...
    public static Intent buildGalleryIntent(String pick, Uri uri, CropParams params) {
        Intent intent;
        if (params.enable) {
            intent = buildCropIntent(pick, uri, params);
        } else {
            intent = new Intent(Intent.ACTION_GET_CONTENT)
                    .setType("image/*")
                    .putExtra(MediaStore.EXTRA_OUTPUT, params.uri);
    ...
    private static Intent buildCropIntent(String action, Uri uri, CropParams params) {
