...
public class CalendarView extends View implements View.OnTouchListener
{
    ...
    public void setHolidayMap(Map<Integer, String> holidayMap)
    {
    ...
    public void setPositionedDate(Date positionedDate)
    {
        this.positionedDate = positionedDate;
    }
    ...
    protected void onDraw(Canvas canvas)
    {
        ...
        //价格最低的前三天
        int[] minDay = getMinPriceDate();

        //绘出日历
        setDateColor(canvas, minDay);
        super.onDraw(canvas);
    }
    ...
    private int[] getMinPriceDate()
    {
        int[] minDay = new int[3];
        if (priceMap != null)
        {
            float[] sortPrice = new float[priceMap.size()];
            int i = 0;
            //初始化数组
            for (int key : priceMap.keySet())
            {
                if (todayIndex == -1)
                {
                    sortPrice[i++] = priceMap.get(key);
                } else
                {
                    if (date[todayIndex] < key)
                        sortPrice[i++] = priceMap.get(key);
                    else
                        sortPrice[i++] = Integer.MAX_VALUE;
                }
            }
            //排序
            for (int j = 0; j < sortPrice.length; j++)
            {
                for (int k = j; k < sortPrice.length; k++)
                {
                    if (sortPrice[j] > sortPrice[k])
                    {
                        float temp = sortPrice[j];
                        sortPrice[j] = sortPrice[k];
                        sortPrice[k] = temp;
                    }
                }
            }
            ...
            //取前三个，如果小于三个，有几个取几个
            int maxSize = sortPrice.length < 3 ? sortPrice.length : 3;
            for (int m = 0; m < maxSize; m++)
            {
                for (int key : priceMap.keySet())
                {
                    if (priceMap.get(key) == sortPrice[m])
                    {
                        if (minDay[m] != key)
                        {
                            minDay[m] = key;
    ...
    private void setDateColor(Canvas canvas, int[] minDay)
    {
        for (int i = 0; i < 42; i++)
        {
            String text = date[i] + "";
            String holiday = holidayMap.containsKey(date[i]) ? holidayMap.get(date[i]) : "";
            String price = priceMap.containsKey(date[i]) ? "￥" + priceMap.get(date[i]) : "";
            boolean workDay = workMap.containsKey(date[i]);
            int color = surface.textColor;
            int holidayColor = Color.WHITE;
            int priceColor;
            int bgColor = Color.WHITE;
            //设置最低价格颜色
            priceColor = getPriceColor(minDay, i);
            if (i < curStartIndex)
            {
                continue;
            } else if (i < todayIndex)
            {
                holidayColor = color = surface.borderColor;
            ...
            } else if (todayIndex != -1 && i == todayIndex)
            {
                ...
                priceColor = color;
            } else if (yellowDateIndex != -1 && i == yellowDateIndex)
            {
                color = surface.todayNumberColor;
                holidayColor = color;
                priceColor = color;
            ...
            if (i > todayIndex)
            {
                int week = getXByIndex(i);
                if (week == 1 || week == 7)
                {
                    if (!workDay)
                    {
                        color = surface.cellDownColor;
                    }
                }
            }
            //设置明年今日以后为灰色
            if (i > nextYearTodayIndex)
            {
                holidayColor = color = surface.borderColor;
                priceColor = Color.WHITE;
            }
            //设置节假日颜色
            boolean isHoliday = !"".equals(holiday);
            boolean afterToday = i > todayIndex;
            if (isHoliday && afterToday)
            {
                color = Color.WHITE;
                bgColor = surface.cellDownColor;
            }

            //设置定位颜色
            if (i == positionedIndex)
            {
                holiday = "";
                price = getPositionText();
                text = date[i] + "";
                priceColor = color = Color.WHITE;
                bgColor = surface.blue;
            }

            drawCellText(canvas, i, text, holiday, price, color, holidayColor, priceColor, bgColor);
        }
    }


    private int getPriceColor(int[] minDay, int i)
    {
        int priceColor = surface.priceColor;
        for (int j = 0; j < minDay.length; j++)
        {
            if (i - curStartIndex + 1 == minDay[j])
                priceColor = surface.cellDownColor;
        }
        return priceColor;
    }
