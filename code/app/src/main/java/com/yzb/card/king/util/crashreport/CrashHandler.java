...
import com.yzb.card.king.util.crashreport.highlight.TextHighLightDecorator;
...
//用来保存配置信息
	...
	public void collectCrashDeviceInfo(Context ctx) {
		try {
			...
			@SuppressLint("WrongConstant") PackageInfo pi = pm.getPackageInfo(ctx.getPackageName(),
	...
	public void uncaughtException(Thread arg0, Throwable arg1) {
		if (debug) {
			String errorMsg = "界面： "
					+ GlobalApp.getInstance().getPublicActivity().getClass().getSimpleName() + "\n";
		...
		dialogInfo.negativeButtonClickListener = new DialogUtil.DialogClicker() {
			...
			public void onClick(DialogInterface dialog, int arg1) {
				...
				if (out != null && !out.equals("")) {
					...
					GlobalApp.getInstance().startActivity(Intent.createChooser(intent,
