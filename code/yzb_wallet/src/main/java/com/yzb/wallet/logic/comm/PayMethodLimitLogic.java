 ...
 * 查询支付方式限额
 */
public class PayMethodLimitLogic extends BasePay {
    ...
    private Boolean showIntegralPay = true;
    private Boolean showDebitCard = true;
    private Boolean showCreditCard = true;

    public PayMethodLimitLogic(Activity activity) {
    ...
    public void addData(List<Map> accountList, List<Map> bankCardList) {
        ...
        if (accountList != null) {
            for (Map<String, String> map : accountList) {
                ...
                map.put("name", "");
                map.put("limitSingle", "50000.00");
                map.put("limitDay", "200000.00");
                map.put("limitMonth", "");
                map.put("logo", "");
                // 判断是否显示账号
